...
function printmsg() {
    ...
    if (usr_input === captcha) {
        document.getElementById("key").innerHTML = "Correspondant";
        document.getElementById("key").style.color = "#98BF64";
        return true;
    } else {
        document.getElementById("captchaInput").value = "";
        document.getElementById("key").innerHTML = "Non Correspondant";
...
function signup() {
    ...
    validateInputs(isFNValid, "firstNameError", "Le prénom doit comporter au moins 3 caractères");

    var lastName = getElementById("lastName");
    var isLNValid = checkLength(lastName, 3);
    validateInputs(isLNValid, "lastNameError", "Le nom doit comporter au moins 3 caractères");

    var email = getElementById("email");
    var isEmailValid = checkEmail(email);
    validateInputs(isEmailValid, "emailError", "L'adresse e-mail n'est pas valide");

    var tel = getElementById("tel");
    var isTelValid = checkTel(tel, 8);
    validateInputs(isTelValid, "telError", "Le numéro de téléphone doit comporter 8 caractères");

    var adress = getElementById("adr");
    var isADRValid = checkLength(adress, 5);
    validateInputs(isADRValid, "adrError", "L'adresse doit comporter au moins 5 caractères");

    var pwd = getElementById("pwd");
    var isPwdValid = checkLength(pwd, 6);
    validateInputs(isPwdValid, "pwdError", "Le mot de passe doit comporter au moins 6 caractères");
...
function signupOwner() {
    ...
    var isFNValid = checkLength(firstName, 3);
    validateInputs(isFNValid, "firstNameOwnerError", "Le prénom doit comporter au moins 3 caractères");

    var lastName = getElementById("lastNameOwner");
    var isLNValid = checkLength(lastName, 3);
    validateInputs(isLNValid, "lastNameOwnerError", "Le nom doit comporter au moins 3 caractères");

    var email = getElementById("emailOwner");
    var isEmailValid = checkEmail(email);
    validateInputs(isEmailValid, "emailOwnerError", "L'adresse e-mail n'est pas valide");

    var tel = getElementById("telOwner");
    var isTelValid = checkTel(tel, 8);
    validateInputs(isTelValid, "telOwnerError", "Le numéro de téléphone doit comporter 8 caractères");

    var adress = getElementById("adrOwner");
    var isADRValid = checkLength(adress, 5);
    validateInputs(isADRValid, "adrOwnerError", "L'adresse doit comporter au moins 5 caractères");

    var pwd = getElementById("pwdOwner");
    var isPwdValid = checkLength(pwd, 6);
    validateInputs(isPwdValid, "pwdOwnerError", "Le mot de passe doit comporter au moins 6 caractères");
...
function signupAdmin() {
    ...
    var isFNValid = checkLength(firstName, 3);
    validateInputs(isFNValid, "firstNameErrorAdmin", "Le prénom doit comporter au moins 3 caractères");

    var lastName = getElementById("lastNameAdmin");
    var isLNValid = checkLength(lastName, 3);
    validateInputs(isLNValid, "lastNameErrorAdmin", "Le nom doit comporter au moins 3 caractères");

    var email = getElementById("emailAdmin");
    var isEmailValid = checkEmail(email);
    validateInputs(isEmailValid, "emailAdminError", "L'adresse e-mail n'est pas valide");

    var pwd = getElementById("pwdAdmin");
    var isPwdValid = checkLength(pwd, 6);
    validateInputs(isPwdValid, "pwdErrorAdmin", "Le mot de passe doit comporter au moins 6 caractères");
...
function login() {
    ...
    if ((findedUser)) {
        if (findedUser.role == "client") {
            ...
            location.replace("indexFr.html");

        } else if (findedUser.role == "owner") {
            //owner not yet verified
            if (findedUser.status == "NOT OK") {
                document.getElementById("login Error").innerHTML = "propriétaire non encore vérifié";
                document.getElementById("login Error").style.color = "#f2b0a5";
            } else {
                document.getElementById("login Error").innerHTML = "";
                localStorage.setItem("connectedUser", JSON.stringify(findedUser.id));
                location.replace("ownerFr.html");
            }
        ...
        document.getElementById("login Error").innerHTML = "identifiant incorrectn";
        document.getElementById("login Error").style.color = "#f2b0a5";
...
function addHouse() {
    ...
    validateInputs(isHouseNameValid, "HouseNameError", "Le nom doit avoir au moins 3 caractères");

    var city = getElementById("guestHouseCity");

    var adr = getElementById("guestHouseAdr");
    var isAdrValid = checkLength(adr, 5);
    validateInputs(isAdrValid, "houseAdrError", "L'adresse doit avoir au moins 5 caractères");

    var area = getElementById("guestHouseArea");
    var isAreaValid = checkNumber(area, 199);
    validateInputs(isAreaValid, "areaError", "La surface doit être d'au moins 200  m²");

    var description = getElementById("guestHouseDescription");
    var isDescriptionValid = checkLength(description, 4);
    validateInputs(isDescriptionValid, "guestHouseDescriptionError", "La description doit avoir au moins 4 caractères");
    ...
    if (isHouseNameValid && isAdrValid && isAreaValid && isDescriptionValid) {
        ...
        location.replace("addRoomFr.html");



    }
}
// Fonction pour afficher dynamiquement les maisons entrées par l'utilisateur
    ...
    for (var i = 0; i < housesTab.length; i++) {
        ...
        <div class="room-item">
            ...
            <div class="ri-text">
                ...
                <a href="#" class="primary-btn" onclick="goToDisplay('${'displayHouseId'}',${housesTab[i].id},'${'roomsFr.html'}')">Display</a>
            </div>
        </div>
    </div>
        `
...
function addRoom() {
    ...
    validateInputs(isRoomNameValid, "roomNameError", "Le nom doit comporter au moins 3 caractères");

    var price = getElementById("pricePerPerson");
    var isPriceValid = checkNumber(price, 0);
    validateInputs(isPriceValid, "roomPriceError", "Le prix doit être d'au moins 1");

    var size = getElementById("roomSize");
    var isSizeValid = checkNumber(size, 15);
    validateInputs(isSizeValid, "roomSizeError", "La surface doit être d'au moins 16 m² ");

    var capacity = getElementById("roomCapacity");
    var isCapacityValid = checkNumber(capacity, 0);
    validateInputs(isCapacityValid, "roomCapacityError", "La capacité de la chambre doit être d'au moins 1");

    var bed = getElementById("roomBed");
    var isBedValid = checkNumber(bed, 0);
    validateInputs(isBedValid, "roomBedError", "La chambre doit avoir au moins 1 lit");

    var description = getElementById("roomDescription");
    var isDescriptionValid = checkLength(description, 4);
    validateInputs(isDescriptionValid, "roomDescriptionError", "La description doit comporter au moins 4 caractères");
    ...
    <div class="col-lg-4 col-md-6">
                <div class="room-item">
                    ...
                    <div class="ri-text">
                        ...
                        <h3>${roomsTab[i].price}DT/Par Personne</h3>
                        <table>
                            <tbody>
                                <tr>
                                    <td class="r-o">Surface:</td>
                                ...
                                <tr>
                                    ...
                                    <td> <span>Max</span>${roomsTab[i].capacity}<span>personne</span></td>
                                </tr>
                                <tr>
                                    <td class="r-o">lit:</td>
                                    <td>${roomsTab[i].bed} lits</td>
                                </tr>
                                <tr>
                                    <td class="r-o">Description:</td>
                        ...
                        <a href="#" class="primary-btn" onclick="goToDisplay('${'displayRoomId'}',${roomsTab[i].id},'${'roomDetailsFr.html'}')">display</a>
                    </div>
                </div>
            </div>
    `
...
function displayRoomDetails() {
    ...
    document.getElementById('roomPriceDetails').innerHTML = roomAdequat.price + " DT/Par personne";
...
function checkAvailability() {
    ...
    if ((entryDate.getTime() < exitDate.getTime()) &&(validDate(dateEntry,dateExit,entryDate,exitDate)) && (guestsNumber <= Number(room.capacity))) {
        ...
        document.getElementById('disponibilityMsg').innerHTML = "La chambre est disponible";
        document.getElementById('disponibilityMsg').style.color = "#98BF64";
        dispo = true;
    } else {
        document.getElementById('disponibilityMsg').innerHTML = "";
        document.getElementById('disponibilityMsg').innerHTML = "La chambre n'est pas disponible";
...
function bookingNow() {
    ...
    if (checkAvailability()) {
        ...
        document.getElementById("msgErr").innerHTML = "Réservation Effectuée";
        document.getElementById("msgErr").style.color = "#98BF64";
    } else {
        document.getElementById('msgErr').innerHTML = "Vérifier la Disponibilité";
        ...
        if (reservationsTab[i].userId == connectedUserId) {
            ...
            content = content + `
                            <tr>
                                ...
                                <td>
                                    <h5>${reservationsTab[i].status ? "confirmé" : "non confirmé"}</h5>
                                </td>
                                <td class="row">
                                <div>
        ...
        if (connectedUser.role == "client") {
            ...
            <nav class="mainmenu">
                <ul>
                    ...
                    <li ><a href="./housesFr.html">Maisons</a></li>
                    <li><a href="./contactFr.html">Contact</a></li>
                    <li><a href="./profileFr.html">Salut ${connectedUser.firstName} ${connectedUser.lastName}!</a></li>
                    <li><a href="./reservationsFr.html">Reservations</a></li>
                    <li><a href="./indexFr.html" onclick='logOut()'>Déconnexion</a></li>
...
`
            ...
            <nav class="mainmenu">
                <ul>
                    ...
                    <li ><a href="./addHouseFr.html">Ajouter une Maison</a></li>
                    <li ><a href="./addRoomFr.html">Ajouter une Chambre</a></li>
                    <li><a href="./profileFr.html">Salut ${connectedUser.firstName} ${connectedUser.lastName}!</a></li>
                    <li class="active"><a href="./ownerFr.html">Tableau de Bord</a></li>
                    <li><a href="./indexFr.html" onclick='logOut()'>Déconnexion</a></li>
...
`
            ...
            <nav class="mainmenu">
                <ul>
                    <li><a href="./indexFr.html">Accueil</a></li>
                    <li><a href="./profileFr.html">Salut admin!</a></li>
                    <li class="active"><a href="./adminFr.html">Tableau de Bord</a></li>
                    <li><a href="./indexFr.html" onclick='logOut()'>Déconnexion</a></li>
        ...
        content = `
                    <nav class="mainmenu">
                        <ul>
                            ...
                            <li><a href="./housesFr.html">Maisons</a></li>
                            <li><a href="./contactFr.html">Contact</a></li>
                            <li><a href="./signupFr.html">Client</a></li>
                            <li><a href="./signupOwnerFr.html">propriétaire</a></li>
                            <li><a href="./loginFr.html" >Connexion</a></li>
...
function displayAdminRooms() {
    ...
    for (var i = 0; i < roomsTab.length; i++) {
        ...
        content = content + `
            ...
            <td>
            <div style="padding-bottom:10px;">
                <button type="button"  class="btn btn-outline-danger" onclick="deleteRoomAdmin(${i})"><i class="fa fa-trash"></i></button>
            </div>
            <div> 
...
function deleteRoomOwner(id) {
    ...
    localStorage.setItem("reservations", JSON.stringify(housesTab));
    localStorage.setItem("houses", JSON.stringify(newRes));
...
function displayProfile() {
    ...
    if (userAdequat.role == "admin") {
        ...
        <table style="margin-top: 75px;" >
            <tbody>
                <tr style="display: flex;">
                    <td class="r-o"><h3 style="color: #708090;">Prénom :</h3></td>
                    <td><h3>${userAdequat.firstName}</h3> </td>
                </tr>
                <tr style="display: flex;">
                    <td class="r-o"><h3 style="color: #708090;">Nom :</h3></td>
        ...
        <div style="padding-left: 125px;">
            <button type="button" class="btn btn-outline-secondary" onclick="EditProfileStore(${userAdequat.id})">Modifier le profil</button>
        </div>
        ...
        <table style="margin-top: 30px;" >
            <tbody>
                ...
                <tr style="display: flex;">
                    <td class="r-o"><h3 style="color: #708090;">Téléphone:</h3></td>
                    <td ><h3>${userAdequat.tel}</h3></td>
                </tr>
                <tr style="display: flex;">
                    <td class="r-o"><h3 style="color: #708090;">Adresse:</h3></td>
        ...
        <div style="padding-left: 125px;">
            <button type="button" style="width:150px;height:50px" class="btn btn-outline-secondary" onclick="EditProfileStore(${userAdequat.id})" >Modifier le profil</button>
        </div>
...
function EditProfileStore(id) {
    ...
    <div class="login_form_inner" style="padding-bottom: 100px" >
	    ...
	    <div class="row login_form" action="contact_process.php" method="post" id="contactForm" novalidate="novalidate">
			...
			<div class="col-md-12 form-group">
                <button type="submit" value="submit" class="primary-btn" onclick="validateEditProfile(${user.id})">valider</button>
...
function goToSearch() {
    location.replace("searchFr.html");
}
...
function goToHousesSearch() {
    ...
    location.replace("housesBySearchFr.html");
...
function search() {
    ...
    if ((trouve == false) &&(i>housesTab.length)){
        document.getElementById("divHouseSearch").innerHTML = "la maison n'existe pas";
